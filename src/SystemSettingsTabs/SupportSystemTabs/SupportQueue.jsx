...
const SupportQueue = () => {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Ticket ID', 'Subject', 'Customer', 'Category', 'Auto-Delist', 'Status', 'Assignee', 'SLA Status'];
    const csvContent = [
      headers.join(','),
      ...dataToExport.map(ticket =>
        [ticket.id, `"${ticket.subject}"`, ticket.customer, ticket.category, ticket.autoDelist, ticket.status, ticket.assignee, ticket.slaStatus].join(',')
      )
    ].join('\n');
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(18);
    doc.text('Support Ticket Queue Report', 14, 20);
    doc.setFontSize(11);
    doc.text(`Generated: ${new Date().toLocaleString()}`, 14, 28);
    doc.text(`Total Tickets: ${dataToExport.length}`, 14, 35);
    ...
    doc.autoTable({
      head: [['Ticket ID', 'Subject', 'Customer', 'Category', 'Priority', 'Status', 'Assignee', 'SLA Status']],
      body: tableData,
      startY: 42,
      styles: { fontSize: 8 },
    ...
    <div className={systemStyles.generalSettingContainer}>
      ...
      <div className={systemStyles.settingSection}>
        ...
        <div className={styles.exportSection}>
          <button className={styles.exportCSV} onClick={exportToCSV}>
            <FileDown size={18} />
            Export CSV
          </button>
          <button className={styles.exportPDF} onClick={exportToPDF}>
            <FileDown size={18} />
            Export PDF
          </button>
          ...
          <button className={styles.auditTrailBtn}>
            Audit Trail
          </button>
        </div>
